...
def skills_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sio.adapters.claude_code.hooks.stop._SKILLS_DIR", d,
    )
    return d
...
def error_log(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sio.adapters.claude_code.hooks.pre_compact._ERROR_LOG", log_path,
    )
    monkeypatch.setattr(
        "sio.adapters.claude_code.hooks.stop._ERROR_LOG", log_path,
    )
    monkeypatch.setattr(
    ...
    def test_counts_errors_from_invocations(self, mem_conn):
        ...
        for i in range(3):
            mem_conn.execute(
                ...
                (
                    "sess-errs", "Bash", "test",
                    "skill", f"2025-01-01T00:00:0{i}Z", "claude-code",
                ),
            )
        ...
        for i in range(2):
            mem_conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?)",
                (
                    "sess-errs", f"2025-01-01T00:00:0{i}Z",
                    "hook", "/tmp/errs.jsonl",
                    "some error", "2025-01-01T00:00:00Z",
                ),
            )
        mem_conn.commit()
    ...
    def test_saves_high_confidence_pattern(self, mem_conn, skills_dir):
        ...
        for i in range(5):
            mem_conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?)",
                (
                    "sess-hi", f"2025-01-01T00:00:0{i}Z",
                    "hook", "/tmp/hi.jsonl",
    ...
    def test_does_not_save_low_confidence_pattern(self, mem_conn, skills_dir):
        ...
        for i in range(5):
            mem_conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?)",
                (
                    "sess-lo", f"2025-01-01T00:00:0{i}Z",
                    "hook", "/tmp/lo.jsonl",
...
class TestUserPromptSubmit:
    ...
    def test_returns_allow_on_normal_message(self, session_state_file):
        ...
        result = json.loads(
            handle_user_prompt_submit(
                payload, state_path=session_state_file,
            ),
        )
        assert result == {"action": "allow"}
    ...
    def test_frustration_logged_after_3_negatives(
        self, session_state_file, error_log,
    ):
        from sio.adapters.claude_code.hooks.user_prompt_submit import (
            handle_user_prompt_submit,
        )
    ...
    def test_returns_allow_on_invalid_json(self, session_state_file):
        ...
        result = json.loads(
            handle_user_prompt_submit(
                "not-json", state_path=session_state_file,
            ),
        )
        assert result == {"action": "allow"}
