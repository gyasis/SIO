...
class TestGetReviewableReturnsUnlabeled:
    ...
    def test_respects_limit(self, tmp_db, sample_invocation):
        _insert_many(
            tmp_db,
            sample_invocation,
            [{"platform": "claude-code", "user_satisfied": None} for _ in range(10)],
        )
...
class TestSkewWarning:
    ...
    def test_skew_warning_when_imbalanced(self, tmp_db, sample_invocation):
        ...
        _insert_many(
            tmp_db,
            sample_invocation,
            [{"platform": "claude-code", "user_satisfied": 1} for _ in range(10)],
        )
        _insert_many(
            tmp_db,
            sample_invocation,
        ...
        _insert_many(
            tmp_db,
            sample_invocation,
            [{"platform": "claude-code", "user_satisfied": None} for _ in range(3)],
        )
        results = get_reviewable(tmp_db, platform="claude-code")
    ...
    def test_no_skew_warning_when_balanced(self, tmp_db, sample_invocation):
        ...
        _insert_many(
            tmp_db,
            sample_invocation,
            [{"platform": "claude-code", "user_satisfied": 1} for _ in range(5)],
        )
        _insert_many(
            tmp_db,
            sample_invocation,
            [{"platform": "claude-code", "user_satisfied": 0} for _ in range(5)],
        )
        _insert_many(
            tmp_db,
            sample_invocation,
