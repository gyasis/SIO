...
class TestGenerateSkillFromPattern:
    ...
    def test_title_derived_from_description(
        self, realistic_pattern, positive_examples
    ):
    ...
    def test_trigger_conditions_include_tool(
        self, realistic_pattern, positive_examples
    ):
    ...
    def test_trigger_conditions_from_error_context(
        self, realistic_pattern, positive_examples
    ):
    ...
    def test_steps_from_flow_sequence(
        self, realistic_pattern, positive_examples
    ):
    ...
    def test_steps_inferred_from_positives(
        self, realistic_pattern, positive_examples
    ):
    ...
    def test_guardrails_negate_error_pattern(
        self, realistic_pattern, positive_examples
    ):
    ...
    def test_provenance_includes_stats(
        self, realistic_pattern, positive_examples
    ):
    ...
    def test_provenance_includes_auto_generated_note(
        self, realistic_pattern, positive_examples
    ):
...
class TestGenerateSkillFromFlow:
    ...
    def test_has_all_required_sections(
        self, flow_ngram_4, session_examples
    ):
        """Output must contain all four required sections."""
        result = generate_skill_from_flow(
            flow_ngram_4, 0.85, session_examples
        )
        assert "# Skill:" in result
        assert "## When to Use" in result
        assert "## Steps" in result
    ...
    def test_steps_match_flow_order(
        self, flow_ngram_4, session_examples
    ):
    ...
    def test_title_contains_flow_arrow_notation(
        self, flow_ngram_4, session_examples
    ):
    ...
    def test_success_rate_in_provenance(
        self, flow_ngram_4, session_examples
    ):
    ...
    def test_session_count_in_provenance(
        self, flow_ngram_4, session_examples
    ):
    ...
    def test_goals_extracted_from_session_examples(
        self, flow_ngram_4, session_examples
    ):
    ...
    def test_slug_sanitized(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            path = write_skill_file(
                "# test", "My Skill / Version: 2", target_dir=tmpdir
            )
...
class TestIntegration:
    ...
    def test_pattern_to_file(self, realistic_pattern, positive_examples):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            path = write_skill_file(
                content, "file-not-found-edit", target_dir=tmpdir
            )
            assert os.path.isfile(path)
            with open(path, encoding="utf-8") as f:
                written = f.read()
    ...
    def test_flow_to_file(self, flow_ngram_4, session_examples):
        ...
        content = generate_skill_from_flow(
            flow_ngram_4, 0.85, session_examples
        )
        with tempfile.TemporaryDirectory() as tmpdir:
            path = write_skill_file(
                content, "read-grep-edit-bash", target_dir=tmpdir
            )
            assert os.path.isfile(path)
            with open(path, encoding="utf-8") as f:
                written = f.read()
