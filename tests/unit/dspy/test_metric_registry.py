...
def test_metric_registry_has_llm_judge_recall():
    ...
    m = _import_metrics()
    assert "llm_judge_recall" in m.METRIC_REGISTRY, "METRIC_REGISTRY missing 'llm_judge_recall'"
...
def test_metric_signature_matches_contract(name):
    ...
    assert trace_param.default is None, f"Metric '{name}' 'trace' parameter must default to None"
...
def test_llm_judge_recall_returns_float_in_range(mock_lm):
    ...
    try:
        ...
        assert isinstance(result, (float, int)), f"Expected float/int, got {type(result).__name__}"
