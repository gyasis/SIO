...
class TestHumanTag:
    ...
    def test_tag_persists_after_approve(self, v2_db):
        ...
        v2_db.execute("UPDATE suggestions SET status = 'approved' WHERE id = ?", (sid,))
        v2_db.commit()
        row = v2_db.execute(
...
class TestAiTagOnSuggestion:
    ...
    def test_stores_ai_explanation(self, v2_db):
        ...
        sid = _seed_suggestion(v2_db)
        result = ai_tag_suggestion(v2_db, sid, _make_pattern(), _make_dataset())
        assert result is True
        row = v2_db.execute(
            "SELECT ai_explanation FROM suggestions WHERE id = ?", (sid,)
        ).fetchone()
    ...
    def test_nonexistent_returns_false(self, v2_db):
        ...
        result = ai_tag_suggestion(v2_db, 9999, _make_pattern(), _make_dataset())
        assert result is False
