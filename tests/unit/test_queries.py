...
class TestGetInvocationById:
    def test_retrieves_inserted_record(self, tmp_db, sample_invocation):
        record = sample_invocation(
            session_id="sess-abc", platform="claude-code", tool_name="Bash", user_message="run ls"
        )
...
class TestGetUnlabeled:
    ...
    def test_respects_limit(self, tmp_db, sample_invocation):
        _insert_many(
            tmp_db,
            sample_invocation,
            [{"platform": "claude-code", "user_satisfied": None} for _ in range(5)],
        )
...
class TestGetBySkill:
    def test_filters_by_actual_action(self, tmp_db, sample_invocation):
        _insert_many(
            tmp_db,
            sample_invocation,
            [{"tool_name": "Read"}, {"tool_name": "Read"}, {"tool_name": "Bash"}],
        )
...
class TestGetBySession:
    def test_filters_by_session_id(self, tmp_db, sample_invocation):
        _insert_many(
            tmp_db,
            sample_invocation,
            [{"session_id": "sess-001"}, {"session_id": "sess-001"}, {"session_id": "sess-002"}],
        )
...
class TestUpdateSatisfaction:
    def test_updates_satisfaction_fields(self, tmp_db, sample_invocation):
        ...
        success = update_satisfaction(
            tmp_db, id=row_id, satisfied=1, note="Great", labeled_by="human"
        )
    ...
    def test_returns_false_for_missing_id(self, tmp_db):
        assert (
            update_satisfaction(tmp_db, id=99999, satisfied=1, note=None, labeled_by="human")
            is False
        )
...
class TestGetLabeledForOptimizer:
    def test_returns_labeled_above_threshold(self, tmp_db, sample_invocation):
        ...
        results = get_labeled_for_optimizer(
            tmp_db, skill="Read", platform="claude-code", min_examples=10
        )
...
class TestCountByPattern:
    ...
    def test_empty_db_returns_zero(self, tmp_db):
        assert (
            count_by_pattern(tmp_db, behavior_type="skill", failure_mode="incorrect_outcome") == 0
        )
