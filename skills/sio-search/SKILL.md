...
OpenCode, Gemini, Aider, PromptChain, Kimi, pi), or narrow every SIO analysis command to one
...
sio search "pattern" --agent pi            # the pi coding agent (~/.pi/agent/sessions)
...
| `pi` | pi coding agent (searchable + live-attachable; `isError` tool results mined as `tool_failure`) |
| `all` | All nine harnesses |
