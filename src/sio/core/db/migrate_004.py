"""sio.core.db.migrate_004 — Apply schema deltas from data-model.md §2.

Applies every additive ALTER TABLE / CREATE INDEX from the
004-pipeline-integrity-remediation spec. Each ALTER TABLE ADD COLUMN is
wrapped in a try/except that silently ignores sqlite3.OperationalError
"duplicate column name" so the migration is fully idempotent.

This lives IN the ``sio`` package (not ``scripts/``) so it is importable
regardless of how ``sio`` was installed. ``scripts/`` is a dev-repo-only
directory — it is explicitly NOT bundled into the wheel (see
``[tool.hatch.build.targets.wheel]`` in ``pyproject.toml``, which only
force-includes ``skills``/``rules``/``docs``) — so ``from scripts.migrate_004
import migrate`` only ever resolved when the current working directory
happened to be the repo root. On every other install layout it raised
``ModuleNotFoundError`` at ``sio.core.db.bootstrap.ensure_canonical_db_ready``
time, which caught it and logged at DEBUG — invisible by default — so the
004 columns (``pattern_errors.active``, ``patterns.active``, etc.) silently
never landed on a fresh ``~/.sio/sio.db``. ``scripts/migrate_004.py`` is now
a thin shim over this module, kept only for the documented
``python -m scripts.migrate_004 <db_path>`` CLI entry point and for the
existing test that loads it by file path.

Usage:
    python -m sio.core.db.migrate_004 ~/.sio/sio.db   # apply to a DB

The migration is recorded as a schema_version row (version=2,
description='004-pipeline-integrity-remediation'). Subsequent calls detect
the existing version=2 'applied' row and are no-ops.
"""
...
def migrate_004(db_path: Path | str) -> None:
...
# Back-compat alias — the migration was historically exposed as `migrate()`
# from `scripts/migrate_004.py`. Keep the short name importable so any
# external caller (or a test that loads the file directly) that still does
# `from scripts.migrate_004 import migrate` / `mod.migrate` keeps working.
migrate = migrate_004
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: python -m sio.core.db.migrate_004 <db_path>", file=sys.stderr)
        return 1
    try:
        migrate_004(sys.argv[1])
